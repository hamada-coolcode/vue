...
const tasksData = ref<Tables<'tasks'>[]>([])
...
onMounted(async () => {
    ...
    console.log('First task collaborations:', data?.[0]?.collaborations)
})
...
const columns: ColumnDef<Tables<'tasks'>>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (!dueDate) return h('span', { class: 'text-muted-foreground/50' }, 'No due date')
            return h('span', {}, new Date(dueDate).toLocaleDateString())
        },
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (!projectId) return h('span', { class: 'text-muted-foreground/50' }, 'Unassigned')
            return h('span', {}, `Project #${projectId}`)
        },
    },
    {
        id: 'collaborators',
        accessorFn: (row: Tables<'tasks'>) => (row as any).collaborators,
