<template>
    ...
    <div v-else-if="task" class="space-y-6">
        <div class="flex items-center gap-4">
            <RouterLink to="/tasks" class="text-blue-500 hover:underline"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1">
                        ...
                        <span v-else class="text-muted-foreground/50">No due date</span>
                    </p>
                </div>

                <div>
                    <h3 class="text-sm font-medium text-muted-foreground">Project</h3>
                    <p class="mt-1">
                        <template v-if="task.project_id"> Project #{{ task.project_id }} </template>
                        <span v-else class="text-muted-foreground/50">Unassigned</span>
                    </p>
                </div>

                <div>
                    <h3 class="text-sm font-medium text-muted-foreground">Created At</h3>
                    <p class="mt-1">{{ new Date(task.created_at).toLocaleDateString() }}</p>
                </div>

                <div>
                    <h3 class="text-sm font-medium text-muted-foreground">Collaborators</h3>
                    <div class="mt-1 flex flex-wrap gap-1">
                        <template v-if="task.collaborators && task.collaborators.length > 0">
                            <span
...
onMounted(async () => {
    ...
    const { data, error: fetchError } = await supabase.from('tasks').select().eq('id', id).single()
